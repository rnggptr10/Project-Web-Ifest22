...
@section('competition_webtitle', 'Data Analysis Competition Analysis')

@section('competition_title', 'Data Analysis Competition - Analysis')

@section('action_form', "")

@section('competition-form')
<!-- PAGE 1 -->
<form method="post" action="{{route('dac.submitting.analysis')}}">
    @method('patch')
    @csrf
    <div id="regist-section-1">
        <div class="row mb-2">
            <label for="analytics_result" class="placeholder regist-form-text" style="padding:0">{{ __('Analysis Link') }}</label>
            <input id="analytics_result" type="text" class="form-control @error('analytics_result') is-invalid @enderror ifest-regist-form text-center" name="analytics_result" value="{{ old('analytics_result') }}" required autocomplete="analytics_result" autofocus placeholder="Insert Analysis Link">
            @error('analytics_result')
