...
@section('competition_webtitle', 'Seminar Nasional Paper 2')

@section('competition_title', 'Seminar Nasional - Paper 2')

@section('action_form', "")

@section('competition-form')
<!-- PAGE 1 -->
<form method="post" action="{{route('semnas.presenter.submitting.paper2')}}">
    @method('patch')
    @csrf
    <div id="regist-section-1">
        <div class="row mb-2">
            <label for="paper2_link" class="placeholder regist-form-text" style="padding:0">{{ __('Paper Link') }}</label>
            <input id="paper2_link" name="paper2_link" type="text" class="form-control @error('paper2_link') is-invalid @enderror ifest-regist-form text-center" value="{{ old('paper2_link') }}" required autocomplete="paper2_link" autofocus placeholder="Insert Paper Link">
            @error('paper2_link')
