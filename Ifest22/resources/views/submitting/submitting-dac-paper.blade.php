...
<form method="post" action="{{route('dac.submitting.paper')}}">
    ...
    <div id="regist-section-1">
        <div class="row mb-2">
            <label for="analytics_result" class="placeholder regist-form-text" style="padding:0">{{ __('Paper Link') }}</label>
            <input id="analytics_result" type="text" class="form-control @error('analytics_result') is-invalid @enderror ifest-regist-form text-center" name="analytics_result" value="{{ old('analytics_result') }}" required autocomplete="analytics_result" autofocus placeholder="Insert Paper Link">
